...
const Topbar = React.memo(({ onToggleSidebar }) => {
  ...
  const [user, setUser] = useState(() => {
    try {
      return JSON.parse(getStorageItem("partnerUser"));
    } catch {
      return null;
    }
  });
  ...
  const fetchNotifs = useCallback(async (force = false) => {
    try {
      const res = await fetchWithCache("/notifications/get-my-notifications?limit=10", { force });
  ...
  const fetchCounts = useCallback(async (force = false) => {
    try {
      ...
      const missionsRes = await fetchWithCache("/local-missions/get-my-local-missions", { force });
      if (missionsRes?.data?.status === "ok") {
        setMyMissionsCount(missionsRes.data.data?.length || 0);
      }
      
      // Get count of partner ads
      const pointsRes = await fetchWithCache("/partner-ads/get-my-partner-ads", { force });
  ...
  useEffect(() => {
    ...
    const handleRefetchCounts = () => {
      fetchCounts(true);
    };

    socket.on("notification:new", handleNewNotif);
    window.addEventListener("refetch-notifications", handleNewNotif);
    window.addEventListener("refetch-counts", handleRefetchCounts);
    
    return () => {
      socket.off("notification:new", handleNewNotif);
      window.removeEventListener("refetch-notifications", handleNewNotif);
      window.removeEventListener("refetch-counts", handleRefetchCounts);
    };
  }, [fetchNotifs, fetchCounts]);
  ...
  useEffect(() => {
    const handleProfileUpdate = () => {
      try {
        const updatedUser = JSON.parse(getStorageItem("partnerUser"));
