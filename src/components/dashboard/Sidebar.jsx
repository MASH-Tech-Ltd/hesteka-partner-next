...
const Sidebar = React.memo(({ isOpen, setIsOpen }) => {
  ...
  const [user, setUser] = React.useState(() => {
    try {
      return JSON.parse(getStorageItem("partnerUser")) || { company: "Partner", firstName: "", lastName: "" };
    } catch {
  ...
  React.useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await fetchWithCache("/user/get-my-profile");
        if (res?.data?.status === "ok" && res.data.data) {
          setUser(res.data.data);
          setStorageItem("partnerUser", JSON.stringify(res.data.data));
  ...
  const fetchStats = React.useCallback(async (force = false) => {
    try {
  ...
  React.useEffect(() => {
    ...
    const handleRefetch = () => fetchStats(true);
