...
export default function SettingsPage() {
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 200 || res.data?.status === "ok") {
        ...
        const storedUser =
          JSON.parse(getStorageItem("partnerUser")) || {};
        const mergedUser = { ...storedUser, ...freshData };
        setStorageItem("partnerUser", JSON.stringify(mergedUser));
    ...
    try {
      ...
      if (results[0]) {
        ...
        for (const component of addressComponents) {
          ...
          if (types.includes("locality") || types.includes("city") || types.includes("town")) {
            city = component.long_name;
          } else if (types.includes("postal_code") || types.includes("postcode")) {
  ...
  useEffect(() => {
    ...
    locationSearchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        const res = await api.get(`/location/autocomplete?input=${encodeURIComponent(locationQuery)}`);
        if (res.data && res.data.status === "ok") {
          setLocationSuggestions(res.data.data || []);
        }
      } catch (err) {
        console.error("Failed to fetch location suggestions from backend:", err);
      } finally {
  ...
  const handleSelectLocation = async (suggestion) => {
    ...
    try {
      ...
      if (res.data && res.data.status === "ok" && res.data.data) {
        ...
        let streetAddress = details.formatted_address || suggestion.description;
        let city = "";
        let postalCode = "";
          ...
          <form onSubmit={handleProfileSubmit} className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5 md:col-span-2">
                ...
                {/* Free Location Autocomplete Search */}
                <div className="relative mb-1 z-50">
                  <input
                    ...
                    onBlur={() => setTimeout(() => setIsLocationSearchFocused(false), 200)}
                    placeholder={t.searchLocationPlaceholder || "Search for an address or city..."}
                    className="w-full bg-[#fcfaf7] border border-[#e8ddd0] rounded-xl pl-10 pr-8 py-2.5 text-xs text-[#3a2a1a] outline-none focus:border-orange-600 transition-all font-medium shadow-sm"
                  />
                  ...
                  {locationQuery && (
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9a8a7a] hover:text-[#3a2a1a] transition-colors"
                      title="Clear"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  ...
                  {isLocationSearchFocused && locationQuery.trim().length >= 2 && (
                    <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl border border-[#e8ddd0] shadow-xl overflow-hidden z-50 max-h-60 overflow-y-auto">
                      {searchingLocation ? (
                        <div className="p-3 flex items-center justify-center gap-2 text-xs text-[#9a8a7a]">
                          <Loader2 className="w-3.5 h-3.5 animate-spin text-orange-600" />
                          Searching OpenStreetMap...
                        </div>
                      ) : locationSuggestions.length === 0 ? (
                        <div className="p-3 text-center text-xs text-[#9a8a7a]">
                        ...
                        locationSuggestions.map((suggestion) => (
                          <div
                            ...
                            className="p-2.5 hover:bg-[#fcfaf7] cursor-pointer border-b border-[#e8ddd0] last:border-b-0 flex items-center gap-2.5 transition-all"
                          >
                            <div className="w-7 h-7 rounded-full bg-[#f5f0e8] flex items-center justify-center text-orange-600 shrink-0 border border-[#e8ddd0]">
                              <MapPin className="w-3.5 h-3.5" />
                            </div>
                            <div className="min-w-0 text-left flex-1">
                              <p className="text-xs font-bold text-[#3a2a1a] truncate">
                                {suggestion.structured_formatting?.main_text || suggestion.description}
                              </p>
                              {suggestion.structured_formatting?.secondary_text && (
                                <p className="text-[10px] text-[#9a8a7a] truncate">
                                  {suggestion.structured_formatting.secondary_text}
                                </p>
                              )}
                            </div>
