...
const getCachedProfile = () => {
  try {
    return JSON.parse(getStorageItem("partnerUser")) || null;
  } catch {
    return null;
  }
};
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async (force = false) => {
    try {
      // Fetch profile, ads, and missions using cache — deduplicates requests with Topbar/Sidebar
      const [profileRes, adsRes, missionsRes] = await Promise.all([
        fetchWithCache("/user/get-my-profile", { force }),
        fetchWithCache("/partner-ads/get-my-partner-ads", { force }),
        fetchWithCache("/local-missions/get-my-local-missions", { force }),
      ]);

      if (profileRes.data.status === "ok") {
        const freshProfile = profileRes.data.data;
        setProfile(freshProfile);
        setStorageItem("partnerUser", JSON.stringify(freshProfile));
      }
