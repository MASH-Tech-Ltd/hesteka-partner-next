...
export default function MissionsPage() {
  ...
  const openCreate = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("openCreate") === "true";
  ...
  const fetchMissions = useCallback(async (isInitial = false, force = false) => {
    try {
      if (isInitial) setLoading(true);
      const res = await fetchWithCache("/local-missions/get-my-local-missions", { force });
  ...
  useEffect(() => {
    ...
    const fetchLocations = async () => {
      try {
        const res = await fetchWithCache("/contacts/locations");
        if (res?.data?.status === "ok") {
  ...
  const handleCrudSubmit = async (formDataObject) => {
    ...
    try {
      ...
      setIsCrudOpen(false);
      invalidateCache("/local-missions");
      fetchMissions(false, true);
      window.dispatchEvent(new Event("refetch-stats"));
      window.dispatchEvent(new Event("refetch-counts"));
    } catch (err) {
