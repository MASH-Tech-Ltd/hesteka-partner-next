  ...
  const fetchPoints = useCallback(
    async (isInitial = false, force = false) => {
      try {
        ...
        const res = await fetchWithCache("/partner-ads/get-my-partner-ads", { force });
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-full bg-[#8B6914] text-white flex items-center justify-center font-bold overflow-hidden border border-[#e8ddd0]">
                      {(() => {
                        try {
                          const user = JSON.parse(getStorageItem("partnerUser") || "{}");
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-[#9a8a7a]">
                        {(() => {
                          try {
                            return JSON.parse(getStorageItem("partnerUser") || "{}").email || "";
                          } catch {
                            return "";
                          }
                        })()}
